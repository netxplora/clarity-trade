...
const TestimonialsSection = () => {
  return (
    <section className="py-24 bg-[#0a0a0a] border-t border-[#333]/50" id="testimonials">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {testimonials.map((testimonial, i) => (
            <motion.div
              ...
              className="bg-[#1A1A1A] border border-[#333] p-8 rounded-xl relative group hover:border-[#D4AF37]/50 transition-all h-full flex flex-col"
            >
              ...
              <div className="flex-grow">
                 <p className="text-gray-300 italic leading-relaxed mb-8">"{testimonial.text}"</p>
              </div>

              <div className="flex items-center gap-4 pt-6 border-t border-[#333]/50 mt-auto">
                <div className="w-12 h-12 rounded-full bg-[#111] flex items-center justify-center text-[#D4AF37] font-extrabold text-sm border border-[#333]">
                   {testimonial.avatar}
                </div>
                <div>
                   <h4 className="font-bold text-white text-base" style={{ fontFamily: "Inter, sans-serif" }}>{testimonial.name}</h4>
                   <span className="text-[10px] uppercase font-bold text-gray-500 tracking-widest">{testimonial.role}</span>
                </div>
              </div>
            </motion.div>
          ))}
