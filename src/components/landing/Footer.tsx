...
const Footer = () => {
  ...
  return (
    <footer className="bg-[#0a0a0a] pt-24 pb-12 border-t border-[#333]/50">
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-16 mb-20 lg:mb-24">
          ...
          <div className="lg:col-span-2 space-y-8">
            <Link to="/" className="flex items-center gap-4 group" onClick={() => window.scrollTo(0,0)}>
              ...
              <span className="text-2xl font-black text-white tracking-tighter leading-none" style={{ fontFamily: "Inter, sans-serif" }}>Clarity<span className="text-[#D4AF37] font-medium">Trade</span></span>
            </Link>
            ...
            <div className="flex items-center gap-4 hidden">
              {[Twitter, Youtube, Linkedin].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded bg-[#1A1A1A] border border-[#333] flex items-center justify-center text-gray-400 hover:text-[#D4AF37] hover:border-[#D4AF37]/50 transition-all">
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category} className="lg:col-span-1">
              <h4 className="font-extrabold text-[#D4AF37] mb-8 text-xs tracking-widest uppercase pb-3 border-b border-[#333]">
                 {category}
              </h4>
                ...
                {links.map((link) => (
                  <li key={link.name}>
                    <Link to={link.href} onClick={() => window.scrollTo(0,0)} className="text-sm text-gray-500 hover:text-[#D4AF37] transition-all flex items-center gap-2 group">
                       <span className="w-4 h-[1px] bg-[#333] group-hover:w-6 transition-all group-hover:bg-[#D4AF37]" /> {link.name}
                    </Link>
                  </li>
                ))}
              </ul>
        ...
        {/* Bottom Banner */}
        <div className="pt-10 border-t border-[#333] flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-wrap items-center justify-center gap-10 opacity-30 grayscale hover:grayscale-0 transition-opacity">
             <div className="text-white font-black text-lg tracking-tighter" style={{ fontFamily: "Inter, sans-serif" }}>FINRA REGULATED</div>
             <div className="text-white font-black text-lg tracking-tighter" style={{ fontFamily: "Inter, sans-serif" }}>SEC COMPLIANCE</div>
             <div className="text-white font-black text-lg tracking-tighter" style={{ fontFamily: "Inter, sans-serif" }}>PCI DSS 1.0</div>
          </div>
        </div>
      </div>
    </footer>
