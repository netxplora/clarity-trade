...
const SubscriptionPlansSection = () => {
  return (
    <section className="py-24 bg-[#0a0a0a] border-t border-[#333]/50" id="investment-plans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {plans.map((plan, i) => (
            <motion.div
              ...
              className={`relative bg-[#1A1A1A] border ${plan.borderColor} rounded-xl p-8 hover:transform hover:-translate-y-2 transition-all duration-300 flex flex-col`}
            >
              <div className={`absolute top-0 left-0 w-full h-32 bg-gradient-to-b ${plan.color} rounded-t-xl opacity-50 pointer-events-none`} />
              
              {plan.popular && (
                <div className="absolute top-0 right-1/2 translate-x-1/2 -translate-y-1/2 bg-[#D4AF37] text-[#0a0a0a] text-xs font-bold uppercase py-1 px-3 rounded-full">
                  Most Popular
                </div>
              )}

              <div className="relative z-10 flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-white" style={{ fontFamily: "Inter, sans-serif" }}>{plan.name}</h3>
              ...
              <div className="relative z-10 flex gap-4 mb-6 pb-6 border-b border-[#333]/50">
                <div className="flex-1">
                  <p className="text-xs text-gray-500 uppercase font-semibold mb-1">Target ROI</p>
                  <p className="text-2xl font-bold text-green-500">{plan.roi}</p>
                </div>
              </div>
              ...
              <div className="relative z-10 flex gap-4 mb-6 space-y-4 flex-col flex-1">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-sm font-semibold text-white">{plan.minInvestment}</p>
                </div>
              ...
              <div className="relative z-10 mt-auto pt-6">
                <Link to="/auth/register" className={`w-full flex items-center justify-center gap-2 py-4 rounded font-semibold transition-colors ${plan.popular ? 'bg-[#D4AF37] text-[#0a0a0a] hover:bg-[#b0902a]' : 'bg-transparent border border-[#333] text-white hover:border-[#D4AF37] hover:text-[#D4AF37]'}`}>
