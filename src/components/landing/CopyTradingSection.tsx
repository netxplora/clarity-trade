...
const CopyTradingSection = () => {
  return (
    <section className="py-24 bg-background border-t border-border/50 relative overflow-hidden" id="copy-trading">
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 max-w-7xl mx-auto">
          ...
          <div className="lg:w-1/3">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mt-2 mb-6" style={{ fontFamily: "Inter, sans-serif" }}>
            ...
            <div className="grid md:grid-cols-2 gap-6 w-full">
              {traders.slice(0, 2).map((trader, i) => (
                <motion.div
                  ...
                  className="bg-card border border-border rounded-xl p-6 hover:border-[#D4AF37]/50 transition-colors"
                >
                  <div className="flex items-center gap-4 mb-6 pb-6 border-b border-border/50">
                    <div className="w-14 h-14 rounded-full bg-secondary border border-border flex items-center justify-center text-foreground font-bold text-xl group-hover:border-[#D4AF37] transition-all">
                      {trader.avatar}
                    </div>
                    <div>
                      <h4 className="font-bold text-foreground text-lg tracking-tight" style={{ fontFamily: "Inter, sans-serif" }}>{trader.name}</h4>
                      <span className="text-xs text-muted-foreground">{trader.specialty} Trader</span>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div>
                       <div className="text-muted-foreground/60 text-xs font-semibold uppercase mb-1">Return (12M)</div>
                       <div className="text-profit font-bold">{trader.roi}</div>
                    </div>
                    <div>
                       <div className="text-muted-foreground/60 text-xs font-semibold uppercase mb-1">Risk Level</div>
                       <div className="text-foreground font-medium">{trader.risk}</div>
                    </div>
                  </div>

                  <Link to="/auth/register" className="w-full h-10 flex items-center justify-center gap-2 group border border-border text-muted-foreground hover:text-[#D4AF37] hover:border-[#D4AF37] bg-transparent rounded font-semibold text-sm transition-all">
